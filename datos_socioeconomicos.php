...
<title>DATOS SOCIOECON&Oacute;MICOS</title>
		...
		/////////////////////////////////////////////////////////////////
		$sql3 = "select estrato_social,estatus_e,ind_acad_t from DACE002 where exp_e='$exp_e'";
		$conex->ExecSQL($sql3,__LINE__,true);
		$resultado = $conex->result;
		$fila_dace = $conex->filas;
		$estrato_social=$conex->result[0][0];
		$estatus_e=$conex->result[0][1];
		$ind_acad_t=$conex->result[0][2];
		...
		/////////////////////////////////////////////////////////////////////////////
		$sql8 = "select distinct U_CREDITOS, D06.C_ASIGNA from TBLACA009 T09, DACE006 D06 where EXP_E='$exp_e' AND D06.LAPSO='$lapsoProceso' AND T09.C_ASIGNA=D06.C_ASIGNA AND 
		STATUS IN(7, 'A')";
		...
		/////////////////////////////////////////////////////////////////////////////
		///////  cursa tesis o practica profesional  ACTUALMENTE/////
		/////////////////////////////////////////////////////////////////////////////
		$sql8x = "select  D06.C_ASIGNA from  DACE006 D06 where EXP_E='$exp_e' AND  
		STATUS IN(7, 'A') and (D06.C_ASIGNA='311939' or  D06.C_ASIGNA='355069' or
		 D06.C_ASIGNA='355959' or D06.C_ASIGNA='344939' or D06.C_ASIGNA='344040' or D06.C_ASIGNA='322040' or D06.C_ASIGNA='322939' or D06.C_ASIGNA='333939' or D06.C_ASIGNA='333040' or D06.C_ASIGNA='311040' )";

		$conex->ExecSQL($sql8x,__LINE__,true);
		//echo $sql8x;
		$resultado8x = $conex->result;
		$fila_uc_actualesx = $conex->filas;
	

		/////////////////////////////////////////////////////////////////////////////
		///////rutina para extraer la info U.C. sumadas CURSADAS ACTUALMENTE )/////
		/////////////////////////////////////////////////////////////////////////////
		/*$sql81 = "select sum(U_CREDITOS)  from TBLACA009 T09, DACE006 D06 where EXP_E='$exp_e' AND D06.LAPSO='$lapsoProceso' AND T09.C_ASIGNA=D06.C_ASIGNA AND STATUS IN(7, 'A')";
echo $sql81."el select";*/

$cSQL = "SELECT SUM(u_creditos) ";
$cSQL.= "FROM tblaca009 a, dace006 b, dace002 c ";
$cSQL.= "WHERE b.lapso='$lapsoProceso' AND b.status IN ('7','A') AND b.exp_e='$exp_e' ";
$cSQL.= " AND a.c_asigna=b.c_asigna AND a.pensum=c.pensum AND b.exp_e=c.exp_e AND a.c_uni_ca=c.c_uni_ca ";

//echo $cSQL;


		$conex->ExecSQL($cSQL,__LINE__,true);
		$resultado81 = $conex->result;
		$fila_uc_sumadas = $conex->filas;
		
		$sumacreditos=$conex->result[0][0];

		$sumacreditos=$uc_actuales;
		//echo $sumacreditos;
//echo $sumacreditos."suma ".$fila_uc_sumadas;
...
### OJO ### OJO ### OJO ### OJO ### OJO ### OJO ### OJO ### OJO ### OJO ###
		...
		if ($lapsoProceso=='2017-1') {$LapsoAnterior = '2016-U';}
...
###########################################################################
				...
				///////////////////rutina para extraer INF. DEL BENEFICIO ANTERIOR//////////////////////
				////////////////////////////////////////////////////////////////////////////////////////
				$sql_Bb = "select TIPO  from AYU_ESTUDIANTES  where CI_E='$ci_e' and LAPSO='$LapsoAnterior'";
			//	echo $sql_Bb;
				$conex->ExecSQL($sql_Bb,__LINE__,true);
				$filabb = $conex->filas;			
				$renov="N";
                if ($filabb != 0){
					$renov="S";
				...
				//////////////////////////////////////////////////////////////////////////////
				///////rutina para extraer materias aprobadas mayor o igual a 7       ///////
				/////////////////////////////////////////////////////////////////////////////
				$sql10 = "select ASIGNATURA,a.c_asigna from TBLACA008 A, DACE004 B where EXP_E='$exp_e' AND A.C_ASIGNA=B.C_ASIGNA AND STATUS=0 and calificacion >6.99 order by asignatura";
				//echo $sql10;
				$conex->ExecSQL($sql10,__LINE__,true);
				...
				//////////////////////////////////////////////////////////////////////////////
				///////rutina para extraer los proyectos                              ///////
				/////////////////////////////////////////////////////////////////////////////
				$sql10a = "select codigo,descripcion from ayu_proyectos order by descripcion";
				$conex->ExecSQL($sql10a,__LINE__,true);
				$result5 = $conex->result;
				$fila5 = $conex->filas;
//echo "proyectos".$fila5;

				//////////////////////////////////////////////////////////////////////////////
				///////rutina para extraer la info del beneficio que esta solicitando///////
				/////////////////////////////////////////////////////////////////////////////
				$sqlll = "select fecha_solicitud,estatus,tipo,lapso,c_asigna,proyecto,hora from ayu_sol_beneficio where EXP_E='$exp_e' AND LAPSO='$lapso' ";
			//	echo $sqlll;
				$conex->ExecSQL($sqlll,__LINE__,true);
				$resultado11 = $conex->result;
				$fila_solicitud = $conex->filas;

				if ($fila_solicitud != 0){//SI HAY REGISTROS
				
					$fecha_solicitud=$conex->result[0][0];
					$estatus_solicitud=$conex->result[0][1];
					$tipo_solicitud=$conex->result[0][2];
					$tipo2=$conex->result[0][2];
					$lapso_solicitud=$conex->result[0][3];
					$codigoA=$conex->result[0][4];
					$materia2 = $conex->result[0][4];
					$proyecto2 =$conex->result[0][5];
					$codigo2=$conex->result[0][5];
					$hora=$conex->result[0][6];
					
				}

				//////////////////////////////////////////////////////////////////////////////
				///////rutina para buscar nombre de asignatura///////
				/////////////////////////////////////////////////////////////////////////////
				$sqlll = "select asignatura from tblaca008 where C_ASIGNA=".$codigoA;
				$conex->ExecSQL($sqlll,__LINE__,true);
				$resul4 = $conex->result;
				$fila4 = $conex->filas;
				if ($fila4 != 0){//SI HAY REGISTROS
						$nombreA=$conex->result[0][0];
				}
				//////////////////////////////////////////////////////////////////////////////
				///////rutina para buscar nombre de proyecto///////
				/////////////////////////////////////////////////////////////////////////////
				$sqllla = "select descripcion from ayu_proyectos where codigo=".$proyecto2;
				$conex->ExecSQL($sqllla,__LINE__,true);
				//echo $sqllla;

				$resul4a = $conex->result;
				$fila4a = $conex->filas;
				//echo "filas".$fila4a;
				if ($fila4a != 0){//SI HAY REGISTROS
						$nombreb=$conex->result[0][0];
				}
...
<form action="envio_regulares.php" method="post" name="valido" >
	...
	<input type="hidden" name="jefe"    value="<?php echo $jefe; ?>" />
	<input type="hidden" name="mat2"    value="<?php echo $materia2; ?>" />
	<input type="hidden" name="proy2"    value="<?php echo $proyecto2; ?>" />
	<input type="hidden" name="renov"    value="<?php echo $renov; ?>" />
	<input type="hidden" name="horab"    value="<?php echo $hora; ?>" />
	<input type="hidden" name="fecha_solicitud"    value="<?php echo $fecha_solicitud; ?>" />
	<input type="hidden" name="estatus_solicitud"    value="<?php echo $estatus_solicitud; ?>" />
	<input type="hidden" name="tipo_solicitud"    value="<?php echo $tipoben; ?>" />
	<input type="hidden" name="materias"    value="<?php echo $materias; ?>" />
	<input type="hidden" name="proyecto"    value="<?php echo $proyecto; ?>" />
	<input type="hidden" name="nombreA"    value="<?php echo $nombreA; ?>" />
	<input type="hidden" name="lapso_solicitud"    value="<?php echo $lapso; ?>" />
	<input type="hidden" name="fila_solicitud"    value="<?php echo $fila_solicitud; ?>" />
...
<?php
	...
	$unaOpcion  = '<option value="1:MENOS DE '. $vUT*$rango[0].' UT">';
...
//	muestrame($rango);
	...
	for($k=0; $k< count($rango) - 1;$k++) {
		$ik++;
		$unaOpcion  = '<option value="'.$ik.':DE '. ($rango[$k]*$vUT+1).' A '. $rango[$k+1]*$vUT.' UT">';
		$unaOpcion .= 'DE '. ($rango[$k]*$vUT+1).' A '. $rango[$k+1]*$vUT.' Bs.F</option>';
		print $unaOpcion;
	}
	$unaOpcion  = '<option value="'.++$ik.':MAS DE '. $vUT*$rango[$k].' UT">';
...
<table width="1000px" border="0" class="titulo" align="center">
<tr>
    <td class="titulo"> <font color= NAVY><B>13. SOLICITUD DE BENEFICIO </B></font></td>
</tr>
</table> 
<?php
	switch ($tipo_solicitud){
		case "1":
			$tipoben = "Preparaduria";
			break;
		case "9":
			$tipoben = "Ayudantia";
			break;
		case "8":
			$tipoben = "Asistente de Investigacion";
			break;
	}	
...
<?php
$inves='';
//echo $fila_uc_actualesx; //cuando es tesista o pasante
//  echo $estatus_e;  1 esta activo
//echo $ind_acad_t."creditos".$sumacreditos;
//$fila_uc_actuales es que esta inscrito
//echo $tipo_solicitud;
// ($estatus_e=='1') esto es que esta activo
// ($fila_uc_actualesx > 0 esto es que es tesista o pasante


if (($estatus_e=='1') or ($fila_uc_actualesx > 0) or ($jefe=='s')){
?>
<table width="1000px" border="0" cellspacing="12" class="datospf" align="center">
<tr>
	<td colspan="2">
		<font color= NAVY> Beneficio a Solicitar </font>
	    <select name="tiposol" size="1" class="datospf" OnChange="beneficio(this.value);" >
		<?php 
		if ($fila_solicitud > 0){ // SI HAY REGISTRO EN LA TABLA 
		?>
			<!-- <option value="<?php echo $tipo_solicitud ?>" selected="selected"> <?php echo $tipoben ?> </option> -->
		<?php 
		} 
		if (($aprobado === false) or ($jefe=='s')) {
		?>
			<option  value=""> -S E L E C C I O N E- </option>
			<option  value="9"> Ayudantia </option>
			...
			if ($ind_acad_t >= '6') {
				...
				$prep='s';
			?>
				<option value="8"> Asistente de Investigacion </option>
				<?php 
				
				if  ($sumacreditos  >= '12' or $fila_uc_actualesx > 0) {
					$prep='s';
				?><!--indice mayor o igual 6  -->
				<option value="1"> Preparaduria </option>
...
if (  $prep=="s") { ?>
	...
	<font color= NAVY>Materias para Preparaduria </font>&nbsp;&nbsp;&nbsp; 
	<select name="materiasp" id="materiasp" class="datospf">
	<?php if ($fila_solicitud > 0){ ?><!--SI HAY REGISTRO EN LA TABLA -->
    <option value="<?php echo $codigoA."-".$nombreA ?>" selected="selected"> <?php echo $nombreA ?> </option>
	<?php 
		} ?>
		...
		?>
		<option value="<?php echo $codigo."-".$asigna;?>"><?php echo $asigna;?></option>
	<?php
...
<td colspan="1">
	...
	<font color= NAVY>Proyectos </font>&nbsp;&nbsp;&nbsp; 
	<select name="proyecto" id="proyecto"  class="datospf" style="width:250px" >
	<?php if ($fila_solicitud > 0){ ?><!--SI HAY REGISTRO EN LA TABLA -->
    <option value="<?php echo $proyecto2."-".$nombreb ?>" selected="selected"> <?php echo $nombreb ?> </option>
	<?php 
		} ?>
		<option selected="selected" value="0"> -SELECCIONE- </option>
		<?php
		for ($p=0; $p<$fila5; $p++){
			...
			$codigo2 = $result5[$p][0];
		?>
		<option value="<?php echo $codigo2."-".$asigna2;?>"><?php echo $asigna2;?></option>
	<?php
		}
	?>
	</select>
...
<?php
	...
	<?php 
	if ($fila_solicitud != 1){
		//$fecha_solicitud = date("d-m-y  h:i:s A");
	...
	?>	
	<td colspan="2">
	<font color= NAVY>Status </font>&nbsp;&nbsp;&nbsp; 
    <input type="text" id="esol" name="esol" size="15" maxlength="15" readonly="readonly" value="<?php echo $esol; ?>" class="datospf"/>
    </td>
	<?php
		switch ($esol){
			case "En Espera":
	...
	if ($jefe=='s') {$estatus_solicitud = "1";}
	?>
	<input type="hidden" name="estatus_solicitud" value="<?php echo $estatus_solicitud ?>"/>
	<td colspan="2">
    <font color= NAVY>Fecha y Hora </font>&nbsp;&nbsp;&nbsp; 
    <input type="text" id="fecha_solicitud" name="fecha_solicitud" size="15" maxlength="15" readonly="readonly" value="<?php echo $fecha_solicitud; ?>" class="datospf"/>
    </td>
</tr> 	
</table>
<?php
